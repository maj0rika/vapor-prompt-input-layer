...
| 코드 품질 (`code-quality`) | tsconfig.app.json `strict: true` + 필수 npm scripts + README 분량 (≥ 500자) | WARN (skip + 안내) |
...
| 0:30 | "검사 실행" 클릭 | "버튼 한 번으로 7개 게이트가 실제 데이터로 채점됩니다." |
...
- **Code Quality 게이트**: tsconfig strict 모드 + 필수 npm scripts + README 분량을 검사한다. bundle size / secret scan 통합은 별도 verify:bundle 스크립트로 운영 중 (게이트로 흡수는 후속 phase).
