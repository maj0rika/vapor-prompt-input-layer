...
Canvas 와 runtime validation 은 `<artifact-meta>`의 `primaryExport`, `defaultProps`,
`variants`를 우선 사용하고, metadata 가 없을 때만 화면에 heuristic preview warning 을
표시합니다. Preview iframe 은 `ready/error` lifecycle 을 parent UI 로 보내며, runtime
mount 실패는 `Canvas runtime: failed` 로 노출됩니다.
